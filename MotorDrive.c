...
//#define     TOUCH_TEST          //触読トルク測定要デバッグOPTION　通常：コメントアウト
...
/* 触読用Time設定値*/
#define     EXCITATING_OFF_MSK_TIME  82      //励磁OFF後のマスク時間(82=10msec)
...
static UB spkReqVrsCheckWait; // spk Vrsチェック要求のWaitフラグ

static UB spkStopStatus; //励磁中のSPK停止指示
static UB excitatingStatusOn; //今まで励磁中だったフラグ
...
void TMR2_Interrupt_SPK(void)
{
    ...
    if (excitatingTime > 0) {//励磁ON時間セットされているなら
        ...
        if (excitatingTime == 0) {
            ...
            spkStopStatus = OFF; //励磁ON時間終了後はSPK再開する
            spkReqVrsCheckWait = ON; //1回wait指示
...
void SetMotorStartSPK(void)
{
    if (excitatingEnableBit & (BIT_MOTOR_M0 | BIT_MOTOR_M1)) {//励磁ONオプションなら
        ...
        spkChangePosition = CHANGE_POSITION_OPEN; //OPEN位置からスタート、ただしショート
...
void SetMotorStopSPK(void)
{
    ...
    spkStopStatus = OFF;
    excitatingStatusOn = OFF;

    SetExcitatingOutOffData(); //励磁パルスが出ていた場合強制OFF

    /**-- 割り込みハンドラをモータパルス出力に戻す --**/
...
void SpkVrsCheck(void)
{
    if ((PIR2bits.C1IF == 1) || (PIR2bits.C2IF == 1)) {
        ...
        ExcitatingOutOn(); //励磁ON
    } else {
        ExcitatingOutOff(); //励磁OFF
    }
...
void ExcitatingOutOff(void)
{
    ...
    /*励磁OFFする際、今まで励磁ONだったなら、マスクカウンタセット*/
    if (excitatingStatusOn) {
        excitatingStatusOn = OFF;

        excitatingOffMaskTime = EXCITATING_OFF_MSK_TIME; //マスクカウンタセット
    }
...
void SetDataTRISxWork_SPK_OPEN(void)
{
    if (detectSpkTurnBit == 0) {// O4側検出
        ...
        dataTRISCWork_M0 = BIT_TRISC_M0_O4_OPN; //O4側OPEN

        dataTRISAWork_M1 = BIT_TRISA_M1_O4_OPN;
        dataTRISBWork_M1 = BIT_TRISB_M1_O4_OPN;
        dataTRISCWork_M1 = BIT_TRISC_M1_O4_OPN; //O4側OPEN
        ...
        dataTRISCWork_M0 = BIT_TRISC_M0_O3_OPN; //O3側OPEN

        dataTRISAWork_M1 = BIT_TRISA_M1_O3_OPN;
        dataTRISBWork_M1 = BIT_TRISB_M1_O3_OPN;
        dataTRISCWork_M1 = BIT_TRISC_M1_O3_OPN; //O3側OPEN
    ...
    if (OFF == spkStopStatus) {//SPK停止指示が無いなら
        spkOutTrisRequestFlug = ON; //TRISX出力要請
    }
...
void SetDataTRISxWork_SPK_SHORT(void)
{
    ...
    spkOutTrisRequestFlug = ON; //SHORT状態のTRISXは無条件に出力させる

    if (OFF == spkStopStatus) {//SPK停止要求ないなら、Vrsチェック要求出す
        if (OFF == spkReqVrsCheckWait) {
            spkReqVrsCheckBit |= (BIT_MOTOR_M0 | BIT_MOTOR_M1); //モータ種別無し Vrsチェック要求
        } else {
            spkReqVrsCheckWait = OFF;
        }
    }
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
